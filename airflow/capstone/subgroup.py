...
if __name__ == '__main__':
    ...
    subgroup = spark.read.option("header", "true").csv("s3://capstonprojecttakeo/capstonData/subgroup.csv")

    has_nulls = subgroup.dropna().count() < subgroup.count()
    if has_nulls:
        print("The dataset has null values.")
    else:
        print("The dataset does not have null values.")

    subgroup.write.format("redshift")\
        .option("url", "jdbc:redshift://default-workgroup.257240804294.us-east-1.redshift-serverless.amazonaws.com:5439/dev")\
        .option("dbtable", "public.subgroup")\
        .option("user", "admin")\
