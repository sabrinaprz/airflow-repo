...
if __name__ == '__main__':
    ...
    group = spark.read.option("header", "true").csv("s3://capstonprojecttakeo/capstonData/group.csv")

    has_nulls = group.dropna().count() < group.count()
    if has_nulls:
        print("The dataset has null values.")
    else:
        print("The dataset does not have null values.")

    group.write.format("redshift").\
        option("url","jdbc:redshift://default-workgroup.257240804294.us-east-1.redshift-serverless.amazonaws.com:5439/dev")\
        .option("dbtable", "public.group").option("user", "admin").option("password", "Nepal123")\
