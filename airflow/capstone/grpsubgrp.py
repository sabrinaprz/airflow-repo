...
if __name__ == '__main__':
    ...
    grpsubgrp = spark.read.option("header", "true").csv("s3://capstonprojecttakeo/capstonData/grpsubgrp.csv")

    has_nulls = grpsubgrp.dropna().count() < grpsubgrp.count()
    if has_nulls:
        print("The dataset has null values.")
    else:
        print("The dataset does not have null values.")

    grpsubgrp.write.format("redshift")\
        .option("url", "jdbc:redshift://default-workgroup.257240804294.us-east-1.redshift-serverless.amazonaws.com:5439/dev")\
        .option("dbtable", "public.grpsubgrp")\
        .option("user", "admin")\
